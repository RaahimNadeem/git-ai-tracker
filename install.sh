...
REPO="RaahimNadeem/git-ai-tracker"
...
detect_std_git() {
    ...
    if [ -z "$git_path" ]; then
        error "Could not detect a standard git binary on PATH. Please ensure you have Git installed and available on your PATH. If you believe this is a bug with the installer, please file an issue at https://github.com/RaahimNadeem/git-ai-tracker/issues."
    fi

    # Verify detected git is usable
    if ! "$git_path" --version >/dev/null 2>&1; then
        error "Detected git at $git_path is not usable (--version failed). Please ensure you have Git installed and available on your PATH. If you believe this is a bug with the installer, please file an issue at https://github.com/RaahimNadeem/git-ai-tracker/issues."
    fi
...
BINARY_NAME="git-ai-tracker-${OS}-${ARCH}"

# Download URL
DOWNLOAD_URL="https://github.com/${REPO}/releases/latest/download/${BINARY_NAME}"

# Install into the user's bin directory ~/.git-ai-tracker/bin
INSTALL_DIR="$HOME/.git-ai-tracker/bin"

# Create directory if it doesn't exist
mkdir -p "$INSTALL_DIR"

# Download and install
echo "Downloading git-ai-tracker..."
TMP_FILE="${INSTALL_DIR}/git-ai-tracker.tmp.$$"
...
mv -f "$TMP_FILE" "${INSTALL_DIR}/git-ai-tracker"

# Make executable
chmod +x "${INSTALL_DIR}/git-ai-tracker"
# Symlink git to git-ai-tracker
ln -sf "${INSTALL_DIR}/git-ai-tracker" "${INSTALL_DIR}/git"
...
if [ "$OS" = "macos" ]; then
    xattr -d com.apple.quarantine "${INSTALL_DIR}/git-ai-tracker" 2>/dev/null || true
fi

PATH_CMD="export PATH=\"$INSTALL_DIR:\$PATH\""

success "Successfully installed git-ai-tracker into ${INSTALL_DIR}"
success "You can now run 'git-ai-tracker' from your terminal"

# Install hooks
echo "Setting up IDE/agent hooks..."
if ! ${INSTALL_DIR}/git-ai-tracker install-hooks; then
    warn "Warning: Failed to set up IDE/agent hooks. Please try running 'git-ai-tracker install-hooks' manually."
else
    success "Successfully set up IDE/agent hooks"
fi

# Write JSON config at ~/.git-ai-tracker/config.json
CONFIG_DIR="$HOME/.git-ai-tracker"
...
if [[ ":$PATH:" != *"$INSTALL_DIR"* ]]; then
    if [ -n "$CONFIG_FILE" ]; then
        ...
        if ! grep -qsF "$INSTALL_DIR" "$CONFIG_FILE"; then
            echo "" >> "$CONFIG_FILE"
            echo "# Added by git-ai-tracker installer on $(date)" >> "$CONFIG_FILE"
