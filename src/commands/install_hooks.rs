    ...
    match check_vscode() {
        Ok(true) => {
            ...
            if binary_exists("code") {
                ...
                match is_vscode_extension_installed("git-ai-tracker.git-ai-tracker-vscode") {
                    Ok(true) => {
                    ...
                    Ok(false) => {
                        if dry_run {
                            ...
                            match install_vscode_extension("git-ai-tracker.git-ai-tracker-vscode") {
                                Ok(()) => {
                                    spinner.success("VS Code: Extension installed");
                                }
                                Err(e) => {
                                    ...
                                    spinner.pending("VS Code: Unable to automatically install extension. Please cmd+click on the following link to install: vscode:extension/git-ai-tracker.git-ai-tracker-vscode (or navigate to https://marketplace.visualstudio.com/items?itemName=git-ai-tracker.git-ai-tracker-vscode in your browser)");
                                }
                            }
                        }
                    }
...
fn is_git_ai_checkpoint_command(cmd: &str, is_post_tool: bool) -> bool {
    // Must contain "git-ai-tracker" and "checkpoint"
    if !cmd.contains("git-ai-tracker") || !cmd.contains("checkpoint") {
        return false;
    }
            ...
            for (idx, existing_hook) in existing_hooks.iter().enumerate() {
                if let Some(existing_cmd) = existing_hook.get("command").and_then(|c| c.as_str()) {
                    ...
                    if existing_cmd.contains("git-ai checkpoint cursor")
                        || existing_cmd.contains("git-ai-tracker")
