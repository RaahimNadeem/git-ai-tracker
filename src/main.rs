...
fn main() {
    ...
    let binary_name = std::env::args_os()
        ...
        .unwrap_or("git-ai-tracker".to_string());

    let cli = Cli::parse();

    #[cfg(debug_assertions)]
    {
        if std::env::var("GIT_AI_TRACKER").as_deref() == Ok("git") {
            commands::git_handlers::handle_git(&cli.args);
            return;
        }
    }

    if binary_name == "git-ai-tracker" || binary_name == "git-ai-tracker.exe" {
