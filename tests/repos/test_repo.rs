...
impl TestRepo {
    ...
    pub fn git_ai(&self, args: &[&str]) -> Result<String, String> {
        ...
        let output = Command::new(binary_path)
            ...
            .output()
            .expect(&format!("Failed to execute git-ai-tracker command: {:?}", args));
...
fn compile_binary() -> PathBuf {
    println!("Compiling git-ai-tracker binary for tests...");
    ...
    let output = Command::new("cargo")
        ...
        .output()
        .expect("Failed to compile git-ai-tracker binary");
    ...
    let binary_path = PathBuf::from(manifest_dir).join("target/debug/git-ai-tracker");
    binary_path
}
...
mod tests {
    ...
    fn test_invoke_git_ai() {
        ...
        let output = repo
            .git_ai(&["version"])
            .expect("git-ai-tracker version should succeed");
