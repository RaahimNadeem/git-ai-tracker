...
$Repo = 'muhammad-nadeem/git-ai-tracker'
...
function Get-StdGitPath {
    ...
    if ($cmd -and $cmd.Path) {
        ...
        if ($cmd.Path -notmatch "git-ai-tracker") {
            $gitPath = $cmd.Path
        }
    }
    ...
    if (-not $gitPath) {
        try {
            $cfgPath = Join-Path $HOME ".git-ai-tracker\config.json"
            if (Test-Path -LiteralPath $cfgPath) {
                $cfg = Get-Content -LiteralPath $cfgPath -Raw | ConvertFrom-Json
                if ($cfg -and $cfg.git_path -and ($cfg.git_path -notmatch 'git-ai-tracker') -and (Test-Path -LiteralPath $cfg.git_path)) {
    ...
    if (-not $gitPath) {
        Write-ErrorAndExit "Could not detect a standard git binary on PATH. Please ensure you have Git installed and available on your PATH. If you believe this is a bug with the installer, please file an issue at https://github.com/RaahimNadeem/git-ai-tracker/issues."
    }
    ...
    try {
        ...
        Write-ErrorAndExit "Detected git at $gitPath is not usable (--version failed). Please ensure you have Git installed and available on your PATH. If you believe this is a bug with the installer, please file an issue at https://github.com/RaahimNadeem/git-ai-tracker/issues."
    }
...
function Set-PathPrependBeforeGit {
    ...
    try {
        ...
        Write-Host 'Your PATH was NOT changed. To ensure git-ai-tracker takes precedence over Git:' -ForegroundColor Red
...
$binaryName = "git-ai-tracker-$os-$arch"
...
# Install directory: %USERPROFILE%\.git-ai-tracker\bin
$installDir = Join-Path $HOME ".git-ai-tracker\bin"
New-Item -ItemType Directory -Force -Path $installDir | Out-Null

Write-Host 'Downloading git-ai-tracker...'
$tmpFile = Join-Path $installDir "git-ai-tracker.tmp.$PID.exe"
...
$finalExe = Join-Path $installDir 'git-ai-tracker.exe'
Move-Item -Force -Path $tmpFile -Destination $finalExe
try { Unblock-File -Path $finalExe -ErrorAction SilentlyContinue } catch { }

# Create a shim so calling `git` goes through git-ai-tracker by PATH precedence
...
try {
    ...
    Write-Warning "Warning: Failed to set up IDE/agent hooks. Please try running 'git-ai-tracker install-hooks' manually."
}

# Update PATH so our shim takes precedence over any Git entries
$scope = Set-PathPrependBeforeGit -PathToAdd $installDir
if ($scope -eq 'Machine') {
    Write-Success 'Successfully added git-ai-tracker to the system PATH.'
} elseif ($scope -eq 'Error') {
    Write-Host 'PATH update failed: system PATH unchanged.' -ForegroundColor Red
}

Write-Success "Successfully installed git-ai-tracker into $installDir"
Write-Success "You can now run 'git-ai-tracker' from your terminal"

# Write JSON config at %USERPROFILE%\.git-ai-tracker\config.json
try {
    $configDir = Join-Path $HOME '.git-ai-tracker'
