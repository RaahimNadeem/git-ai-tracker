...
// Store the git-ai-tracker version for later use
let gitAiVersion: string | null = null;

// Function to get the stored git-ai-tracker version
...
class AIDetector {
  ...
  constructor() {
    ...
    vscode.commands.registerCommand(
      "cursor-git-ai-tracker.pasteWithDetection",
      async () => {
    ...
    vscode.commands.registerCommand(
      "cursor-git-ai-tracker.undoWithDetection",
      async () => {
    ...
    vscode.commands.registerCommand(
      "cursor-git-ai-tracker.redoWithDetection",
      async () => {
...
export function activate(context: vscode.ExtensionContext) {
  // Check if git-ai-tracker CLI is installed
  exec("git-ai-tracker --version", (error, stdout, stderr) => {
    if (error) {
      ...
      // Show startup notification
      vscode.window.showInformationMessage(
        `🤖 AI Code Tracker is now active! (git-ai-tracker v${gitAiVersion})`
      );
    }
  });
}
  ...
  return new Promise<boolean>((resolve, reject) => {
    ...
    exec(
      gitAiVersion?.startsWith("1.")
        ? `git-ai-tracker checkpoint ${author === "ai" ? "cursor" : ""}`
        : // legacy pre 1.0.0
          `git-ai-tracker checkpoint ${author === "ai" ? "--author cursor" : ""}`,
      { cwd: workspaceRoot },
      (error, stdout, stderr) => {
        if (error) {
          if (showCheckpointMessage()) {
